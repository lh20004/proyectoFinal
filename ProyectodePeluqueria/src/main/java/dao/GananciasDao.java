...
public class GananciasDao {
    ...
    public static final String Gananciaspordia = "SELECT CONCAT(c.nombre, ' ', c.apellido) AS cliente, r.fechareserva AS fecha, s.servicio AS servicio, s.precio AS total FROM cliente c JOIN reserva r ON c.idcliente = r.idcliente JOIN detallereserva d ON r.idreserva = d.idreserva JOIN servicio s ON d.idservicio = s.idservicio WHERE r.estado = 'pagado' AND r.fechareserva = ?";
    public static final String Gananciasporsemana = "SELECT CONCAT(c.nombre, ' ', c.apellido) AS cliente, r.fechareserva AS fecha, s.servicio AS servicio, s.precio AS total FROM cliente c JOIN reserva r ON c.idcliente = r.idcliente JOIN detallereserva d ON r.idreserva = d.idreserva JOIN servicio s ON d.idservicio = s.idservicio WHERE r.estado = 'pagado' AND r.fechareserva BETWEEN DATE_TRUNC('week', CAST(? AS DATE)) AND (DATE_TRUNC('week', CAST(? AS DATE)) + INTERVAL '6 days')";
    public static final String Gananciaspormes = "SELECT CONCAT(c.nombre, ' ', c.apellido) AS cliente, r.fechareserva AS fecha, s.servicio AS servicio, s.precio AS total FROM cliente c JOIN reserva r ON c.idcliente = r.idcliente JOIN detallereserva d ON r.idreserva = d.idreserva JOIN servicio s ON d.idservicio = s.idservicio WHERE TO_CHAR(r.fechareserva, 'YYYY-MM') = ? AND r.estado = 'pagado'";
    public GananciasDao() {
